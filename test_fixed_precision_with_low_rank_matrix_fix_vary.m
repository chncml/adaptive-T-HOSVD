...
%     for sample = 1 : 10
%         sample;
%         t1 = tic;[Q1, B1, k1] = randQB_EI_auto_uniform(Aten, err, 50, 1);t1 = toc(t1);
%         t2 = tic;[Q2, B2, k2] = randQB_EI_auto_uniform_fix(Aten, err, 50, 1);t2 = toc(t2);
...
%         t6 = tic;[Q6, B6, k6] = randQB_EI_auto_kr_uniform_fix(Aten, err, 50, 1);t6 = toc(t6);
...
[sum(ERR1, 2), sum(ERR2, 2), sum(ERR3, 2), sum(ERR4, 2), sum(ERR5, 2), sum(ERR6, 2), sum(ERR7, 2), sum(ERR8, 2)]/10
% format short
% [sum(T1, 2), sum(T2, 2), sum(T5, 2), sum(T6, 2)]/10
% [sum(K1, 2), sum(K2, 2), sum(K5, 2), sum(K6, 2)]/10
% format short e
% [sum(ERR1, 2), sum(ERR2, 2), sum(ERR5, 2), sum(ERR6, 2)]/10
end